...
export async function generateProjectZip(
  ...
  // ── Permissions ───────────────────────────────────────────────────────────────
  if (result.permissionProject) {
    const pp = result.permissionProject;
    const pRoot = `${projectRoot}/permissions`;

    // Permission types
    zip.file(`${pRoot}/types.ts`, pp.permissionTypes);

    // Permissions object
    zip.file(`${pRoot}/permissions.ts`, pp.permissionsObject);

    // RBAC middleware
    zip.file(`${pRoot}/rbac-middleware.ts`, pp.rbacMiddleware);

    // Seed script
    zip.file(`${pRoot}/seed-roles.ts`, pp.seedScript);

    // Permissions README
    zip.file(`${pRoot}/README.md`, buildPermissionsReadme(result));
  }

  // ── Notifications ───────────────────────────────────────────────────────────────
  if (result.notificationProject) {
    const np = result.notificationProject;
    const nRoot = `${projectRoot}/notifications`;

    // Email templates
    const templatesFolder = zip.folder(`${nRoot}/email-templates`);
    if (templatesFolder) {
      np.emailTemplates.forEach((tmpl, _i) => {
        const safeName = tmpl.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
        templatesFolder.file(`${safeName}.html`, tmpl.html);
        templatesFolder.file(`${safeName}.txt`, tmpl.text);
        templatesFolder.file(`${safeName}.json`, JSON.stringify({
          name: tmpl.name,
    ...
    zip.file(`${nRoot}/notification-service.ts`, np.notificationService);

    // Notification center component
    zip.file(`${nRoot}/NotificationCenter.tsx`, np.notificationCenter);

    // Notification types
    zip.file(`${nRoot}/notification-types.ts`, np.notificationTypes);

    // Notification store
    zip.file(`${nRoot}/notification-store.ts`, np.notificationStore);
...
function buildRootReadme(result: GenerationResult): string {
  ...
  if (result.permissionProject) {
    lines.push("├── permissions/      # RBAC roles, permissions matrix, and auth middleware");
    lines.push("│   ├── types.ts             # TypeScript types: Role, Permission, PermissionCheck");
  ...
  if (result.notificationProject) {
    ...
    lines.push("│   ├── notification-service.ts  # Multi-provider notification dispatch");
    lines.push("│   ├── NotificationCenter.tsx    # React bell icon dropdown component");
    lines.push("│   ├── notification-types.ts     # TypeScript types: Notification, Template, Preferences");
    lines.push("│   ├── notification-store.ts     # Zustand-compatible state management");
    lines.push("│   └── email-templates/          # Welcome, status change, action required, digest");
  }
  lines.push("```");
  lines.push("");
...
function buildPermissionsReadme(result: GenerationResult): string {
  const pp = result.permissionProject;
  if (!pp) return "# Permissions\n\nNo permissions generated.\n";

  const lines: string[] = [];
  lines.push("# Permissions — RBAC Roles, Matrix, and Middleware");
  lines.push("");
  lines.push("Generated by Genesis Platform based on your business requirements.");
  lines.push("");
  ...
  lines.push("|------|-------------|-------------|");
  for (const role of pp.roles) {
    const permCount = role.permissions.length;
    const actionCount = role.permissions.reduce((sum, p) => sum + p.actions.length, 0);
    lines.push(`| **${role.name}** | ${role.description} | ${permCount} entities × ${actionCount} actions |`);
  }
  lines.push("");

  lines.push("## Permissions Matrix");
  lines.push("");
  lines.push(pp.permissionsMatrix);
  lines.push("");
  ...
  lines.push("| File | Description |");
  lines.push("|------|-------------|");
  lines.push("| `types.ts` | TypeScript type definitions for Role, Permission, PermissionCheck, and role hierarchy |");
  lines.push("| `permissions.ts` | Role → Entity → Actions permissions object with `hasPermission()` helper |");
  lines.push("| `rbac-middleware.ts` | Hono middleware with JWT/API key extraction, `requireRole()`, `requirePermission()`, and `autoEnforceRbac()` |");
  lines.push("| `seed-roles.ts` | Idempotent seed script that creates/updates default roles in the database |");
  lines.push("");

  lines.push("## Integration");
  lines.push("");
  lines.push("### Middleware (Hono)");
  lines.push("");
  lines.push("```typescript");
  lines.push("import { rbacMiddleware, autoEnforceRbac, requirePermission } from './permissions/rbac-middleware';");
  lines.push("");
  lines.push("// Option 1: Manual permission check on routes");
  lines.push("router.get('/api/bookings', requirePermission('Booking', 'read'), handler);");
  lines.push("");
  lines.push("// Option 2: Auto-enforce on all routes");
  lines.push("router.use('/api/*', rbacMiddleware, autoEnforceRbac);");
  lines.push("```");
  lines.push("");
  lines.push("### Permissions Checker");
  lines.push("");
  lines.push("```typescript");
  lines.push("import { rolePermissions, hasPermission } from './permissions/permissions';");
  lines.push("");
  lines.push("if (hasPermission('editor', 'Booking', 'update')) {");
  lines.push("  // User can update bookings");
  lines.push("}");
  lines.push("```");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("bun run permissions/seed-roles.ts");
  lines.push("```");
  lines.push("");
  lines.push("*Generated by [Genesis Platform](https://genesis-platform.com) — Autonomous Software Generation*");
  return lines.join("\n");
...
function buildNotificationsReadme(result: GenerationResult): string {
  const np = result.notificationProject;
  if (!np) return "# Notifications\n\nNo notifications generated.\n";

  const lines: string[] = [];
  lines.push("# Notifications — Email Templates, Service, and UI Component");
  lines.push("");
  lines.push("Generated by Genesis Platform based on your business requirements.");
  lines.push("");
  ...
  lines.push("|----------|---------|");
  for (const tmpl of np.emailTemplates) {
    lines.push(`| **${tmpl.name}** | ${tmpl.subject} |`);
  }
  lines.push("");
  ...
  lines.push("| File | Description |");
  lines.push("|------|-------------|");
  lines.push("| `notification-service.ts` | Multi-provider dispatch: email (Resend/SendGrid/SMTP/SES), push (FCM/APNs/WebPush), in-app (WS/SSE/Polling), webhooks (Slack/Teams/Discord) |");
  lines.push("| `NotificationCenter.tsx` | React component: bell icon with unread badge, dropdown list, type filters, mark-read, delete |");
  lines.push("| `notification-types.ts` | TypeScript interfaces: Notification, NotificationTemplate, NotificationPreferences, DeliveryChannel |");
  lines.push("| `notification-store.ts` | Zustand-compatible store: fetch, markRead, markAllRead, delete, filter, preferences |");
  lines.push("| `email-templates/` | 4 HTML email templates with inline CSS (dark-friendly) and plain-text fallbacks |");
  lines.push("");

  lines.push("## Integration");
  lines.push("");
  lines.push("### Notification Service");
  lines.push("");
  lines.push("```typescript");
  lines.push("import { notificationService } from './notifications/notification-service';");
  lines.push("");
  lines.push("// Send email");
  lines.push("await notificationService.sendEmail({ to: 'user@example.com', subject: 'Welcome!', html: '<h1>Hi</h1>', text: 'Hi' });");
  lines.push("");
  lines.push("// Send push notification");
  lines.push("await notificationService.sendPush({ userId: '123', title: 'New Update', body: 'Your report is ready.' });");
  lines.push("");
  lines.push("// Send in-app notification");
  lines.push("await notificationService.sendInApp({ userId: '123', type: 'success', title: 'Done!', message: 'Task completed.' });");
  lines.push("");
  lines.push("// Dispatch webhook to Slack");
  lines.push("await notificationService.dispatchWebhook({ target: 'slack', title: 'Alert', message: 'Something happened' });");
  lines.push("```");
  lines.push("");
  lines.push("### Notification Center (React)");
  lines.push("");
  lines.push("```tsx");
  lines.push("import { NotificationCenter } from './notifications/NotificationCenter';");
  lines.push("");
  lines.push("// Add to your header/navbar");
  lines.push("<Header>");
  lines.push("  <NotificationCenter />");
  lines.push("</Header>");
  lines.push("```");
  lines.push("");
  lines.push("*Generated by [Genesis Platform](https://genesis-platform.com) — Autonomous Software Generation*");
